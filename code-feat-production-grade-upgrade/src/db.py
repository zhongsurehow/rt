...
class DatabaseManager:
    ...
    async def _create_continuous_aggregates(self, conn: asyncpg.Connection):
        ...
        # Add a policy to automatically refresh the view (only if it doesn't exist)
        # Check more specifically for continuous aggregate policies
        ...
        if not policy_exists:
            ...
            """)
            print("Continuous aggregate policy created for ohlcv_1min.")
        else:
            print("Continuous aggregate policy already exists for ohlcv_1min.")
